...
const Slider: React.FC<{ data: any, index: number, val: number }> = ({ data, index, val }) => {
    ...
    const { name, mothlyFee, planType, nextBillingDate, icon } = data;
    return (
        <Paper
        ...
        >
            ...
            <Stack paddingRight={3} paddingLeft={6}>
                ...
                <Stack direction={'row'} sx={{ position: 'relative', padding: '1.5rem 0' }}>
                    <Stack>
                        <Avatar src={icon} sx={{ width: '7rem', height: '7rem', borderRadius: '3.5rem', marginBottom: '10px' }} />
                ...
                <Stack direction={'row'} paddingBottom={2} flexWrap={'wrap'}>
                    <Stack direction={'row'}>
